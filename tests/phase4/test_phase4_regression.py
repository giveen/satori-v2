...
SNAP_DIR = Path(__file__).parent.parent / 'expected_phase4_snapshots'
...
def test_phase4_regression(fixture_path: Path, update_snapshots: bool):
    ...
    by_id = {h.get('host_id') or h.get('id') or f'host_{i}': h for i, h in enumerate(out_corr)}

    failures = []

    for hid, h in sorted(by_id.items()):
        block = {'correlated_os_inference': h.get('correlated_os_inference'), 'correlation_notes': h.get('correlation_notes')}
        ...
        if not snap_path.exists():
            if update_snapshots:
                ...
                pytest.fail(f"Missing Phase4 snapshot for host {hid}: {snap_path}\nRun with --update-snapshots to create it.")
        else:
    ...
    # also snapshot/write fixture-level metrics JSON
    # metrics snapshot path: <snapdir>/<fixture_stem>__metrics.json
    metrics_block = {'summary_metrics': {
        'num_hosts': len(by_id),
        'num_hosts_with_phase4_inference': sum(1 for h in by_id.values() if h.get('correlated_os_inference') and h['correlated_os_inference'].get('candidates'))
    }}
    ...
    if not metrics_snap.exists():
        if update_snapshots:
            ...
            pytest.fail(f"Missing Phase4 metrics snapshot for fixture {fixture_path.stem}: {metrics_snap}\nRun with --update-snapshots to create it.")
    else:
